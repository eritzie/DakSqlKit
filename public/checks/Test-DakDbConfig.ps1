...
function Test-DakDbConfig {
    ...
    begin {
        ...
        $priority = @{
            "1.1" = "High"    # Unlimited memory causes OS paging under load
            "1.2" = "Medium"  # Unthrottled parallelism degrades OLTP concurrency
            "1.3" = "Medium"  # Default cost threshold of 5 too aggressive for modern hardware
            "1.4" = "Low"     # Backup compression reduces storage and I/O overhead
            "1.5" = "Low"     # Ad hoc plan caching prevents plan cache bloat
            "1.6" = "High"    # No IFI means zero-initialization blocks threads during data file growth
            "1.7" = "Medium"  # No LPIM means buffer pool can be paged out under memory pressure
            "1.8" = "Medium"  # Balanced power plan throttles CPU; High Performance is SQL Server best practice
            "1.9" = "Low"     # Default 6 log files may not retain enough history for incident review
            "2.1" = "High"    # Auto-shrink causes fragmentation and I/O storms
            "2.2" = "Medium"  # Auto-close drops connection pool; performance hit on reconnect
            "2.3" = "High"    # CHECKSUM detects storage corruption; TornPage misses silent corruption
            "2.4" = "Medium"  # Old compat level blocks optimizer improvements
            "2.5" = "Medium"  # Missing auto-create stats leads to poor query plans
            "2.6" = "Medium"  # Stale stats leads to poor query plans
            "3.1" = "Medium"  # Single TempDB file causes allocation contention on multi-core
            "3.2" = "Medium"  # Unequal TempDB files defeat proportional fill; contention returns
            "3.3" = "High"    # Percent autogrowth can trigger multi-GB growth events and VLF explosion
            "3.4" = "Medium"  # Data/log on system drive risks OS stability if drive fills
            "4.1" = "High"    # Enabled SA login is a direct brute-force target
            "4.2" = "Medium"  # Renamed SA is harder to target; defense in depth
            "4.3" = "High"    # TRUSTWORTHY enables dangerous cross-DB ownership chaining
            "4.4" = "Medium"  # Guest CONNECT allows any authenticated login to query the database
            "4.5" = "Medium"  # Orphaned users hold permissions but cannot authenticate
            "4.6" = "Low"     # Excess public server permissions broaden attack surface
            "1.10"= "High"    # Unpatched instances are exposed to known CVEs; end-of-support builds have no fixes
            "1.11"= "High"    # SQL Server running as SYSTEM gives the engine full OS control; use a dedicated service account
            "4.7" = "High"    # Mixed Mode auth exposes SQL logins to brute-force; Windows-only is more secure
            "4.8" = "Medium"  # Missing SPNs force NTLM fallback; breaks Kerberos delegation and double-hop
            "4.9" = "High"    # xp_cmdshell enables OS command execution directly from SQL; highest attack surface
            "4.10"= "Medium"  # CLR enables .NET code execution in SQL; unnecessary attack surface if not used
            "4.11"= "Medium"  # Ole Automation allows COM object instantiation from SQL; unnecessary surface area
            "4.12"= "Medium"  # Ad Hoc Distributed Queries enables OPENROWSET/OPENDATASOURCE without restrictions
            "4.13"= "Medium"  # Cross-DB ownership chaining allows implicit privilege escalation across database boundaries
            "4.14"= "Low"     # Database Mail XPs expose external email infrastructure from SQL context; disable if not used
            "4.15"= "Low"     # Remote access is a deprecated protocol; disable to reduce exposure from legacy RPC connections
            "4.16"= "Low"     # Startup procedures execute at engine start with elevated context; unnecessary ones are a risk
            "4.17"= "High"    # SQL logins in sysadmin bypass all authorization controls; Windows groups preferred
            "4.18"= "Medium"  # Databases owned by SA SID grant elevated context if the owner is impersonated
            "4.19"= "High"    # Linked server SQL auth stores credentials in sys.linked_logins; prefer Windows auth
            "4.20"= "High"    # SQL logins without CHECK_POLICY bypass Windows password complexity and expiration requirements
            "4.21"= "Medium"  # No login failure auditing means brute-force attempts go undetected in the SQL Server error log
            "4.22"= "Medium"  # Priority boost can starve the OS and cause instability; not supported and not recommended
            "4.23"= "Medium"  # Lightweight pooling (fiber mode) is deprecated and unsupported in SQL Server 2019+
            "4.24"= "High"    # Expired or missing TLS certificate causes connection failures and leaves data in transit unprotected
            "4.25"= "High"    # Without force encryption clients may connect unencrypted regardless of server certificate configuration
            "5.1" = "High"    # 4KB allocation unit wastes I/O on SQL Server 8KB pages; 64KB is standard
            "5.2" = "High"    # Data/log on same volume risks data loss if volume fills; also I/O contention
            "5.3" = "Medium"  # TempDB on shared volume competes with user DB I/O under load
            "5.4" = "High"    # Low disk space on SQL volumes risks instance crashes and data loss from log-full conditions
            "6.1" = "High"    # VLF count >1000 causes slow recovery, slow log backup, and slow DB attach
            "6.2" = "High"    # CHECKDB not run in >30 days means undetected corruption may be unrecoverable
            "6.3" = "High"    # Full backup older than 7 days violates typical RPO; data loss window too wide
            "6.4" = "High"    # FULL recovery without frequent log backups leaves large RPO gap between full backups
            "6.5" = "High"    # SUSPECT/EMERGENCY databases indicate unrecoverable errors; require immediate DBA attention
            "7.1" = "High"    # Agent stopped means all scheduled jobs and alerts are silently not executing
            "7.2" = "Medium"  # Severity 17-25 alerts missing means critical engine errors go unnoticed
            "7.3" = "Medium"  # Error 825 (I/O soft error) is an early warning of disk failure
            "7.4" = "Medium"  # No enabled operator means alerts fire but no one receives notification
            "7.5" = "Medium"  # Alert with no notification configured fires but takes no action
            "7.6" = "Medium"  # system_health XE session captures deadlocks, memory errors, and connectivity failures; stopping it eliminates key diagnostic data
            "1.12"= "High"    # Memory dump files indicate SQL Server crashes or internal assertions — each one warrants investigation
            "2.7" = "Medium"  # Collation mismatch causes implicit conversions, sort order inconsistencies, and join collation conflict errors
            "6.6" = "Critical"# Suspect pages indicate unresolved I/O errors, bad checksums, or torn pages — potential data loss
            "6.7" = "High"    # Identity columns near their data type maximum will cause INSERT failures when the type overflows
            "7.7" = "High"    # Failed jobs may indicate missed backups, maintenance failures, or ETL errors that compound over time
            "7.8" = "Medium"  # Jobs significantly exceeding normal run time may indicate blocking, resource contention, or runaway processes
            "7.9" = "Low"     # Ola Hallengren's solution is the de facto standard for backup, integrity check, and index/statistics maintenance
            "8.1" = "High"     # Documents standalone state; no AG protection on this instance
            "8.2" = "High"     # Database mirroring is deprecated (SQL 2012) and removed (SQL 2022)
            "8.3" = "Critical" # Disconnected replicas cannot receive log and will diverge from primary
            "8.4" = "High"     # Unsynchronized replicas increase data loss exposure on failover
            "8.5" = "Medium"   # Manual-only failover requires DBA intervention during an outage
            "8.6" = "High"     # High lag on synchronous replicas blocks primary commits; high async lag raises RPO
            "8.7" = "Medium"   # Without a listener, clients must be reconfigured manually after every failover
            "9.1" = "Info"     # Detection only — documents whether log shipping is in use on this instance
            "9.2" = "High"     # Backup outside threshold means secondary is falling behind the primary
            "9.3" = "High"     # Restore outside threshold means the configured RPO is being exceeded
            "9.4" = "High"     # Log shipping errors indicate jobs failing or latency building undetected
            "1.13"= "Low"     # SQL Browser required for named instance port discovery; not running prevents client connections
            "1.14"= "Critical"# Version approaching or past end-of-support receives no security patches
            "1.15"= "Medium"  # Non-rotating service account credentials increase exposure window on compromise
            "2.8" = "Medium"  # Query Store provides query plan history and regression detection without additional tooling
            "2.9" = "Medium"  # Contained database authentication bypasses instance-level login controls and policy enforcement
            "4.26"= "Info"    # Documents TDE status; encryption requirement depends on data classification and compliance mandate
            "4.27"= "High"    # Without SQL Server Audit there is no tamper-evident record of privileged operations or schema changes
            "4.28"= "Medium"  # Undocumented global trace flags alter engine behavior for all connections and are unsupported
            "4.29"= "High"    # Unrestricted inbound access to SQL port exposes the instance to brute-force and CVE exploitation
            "6.8" = "High"    # Untested backups provide false assurance; the first real recovery attempt should not be the first test
            "7.10"= "Medium"  # Database Mail required for SQL Agent operator notifications; without it alert notifications are silently discarded
            "10.1"= "High"    # An untested or undocumented DR runbook provides false assurance about recovery capability
            "10.2"= "Medium"  # Without an application connectivity map, login changes and database moves carry unknown blast radius
        }

        $sql = @{
            "1.1" = "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'max server memory (MB)';"
            "1.2" = "-- Automated via Test-DbaMaxDop. T-SQL: SELECT [value_in_use] FROM [sys].[configurations] WHERE [name] = 'max degree of parallelism';"
            "1.3" = "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'cost threshold for parallelism';"
            "1.4" = "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'backup compression default';"
            "1.5" = "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'optimize for ad hoc workloads';"
            "1.6" = "-- Automated via Get-DbaPrivilege -ComputerName. Grant 'Perform volume maintenance tasks' to the SQL Server service account in secpol.msc."
            "1.7" = "-- Automated via Get-DbaPrivilege -ComputerName. Grant 'Lock pages in memory' to the SQL Server service account in secpol.msc."
            "1.8" = "-- Automated via Test-DbaPowerPlan -ComputerName. CMD: powercfg /query SCHEME_MIN."
            "1.9" = "-- Automated via Get-DbaErrorLogConfig. EXEC xp_instance_regread N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'NumErrorLogs';"
            "2.1" = "SELECT [name] FROM [sys].[databases] WHERE [is_auto_shrink_on] = 1 AND [database_id] > 4;"
            "2.2" = "SELECT [name] FROM [sys].[databases] WHERE [is_auto_close_on] = 1 AND [database_id] > 4;"
            "2.3" = "SELECT [name], [page_verify_option_desc] FROM [sys].[databases] WHERE [page_verify_option_desc] <> 'CHECKSUM' AND [database_id] > 4;"
            "2.4" = "SELECT [d].[name], [d].[compatibility_level] FROM [sys].[databases] [d] CROSS JOIN (SELECT [compatibility_level] FROM [sys].[databases] WHERE [name] = 'master') [m] WHERE [d].[database_id] > 4 AND [d].[compatibility_level] < [m].[compatibility_level] - 10;"
            "2.5" = "SELECT [name] FROM [sys].[databases] WHERE [is_auto_create_stats_on] = 0 AND [database_id] > 4;"
            "2.6" = "SELECT [name] FROM [sys].[databases] WHERE [is_auto_update_stats_on] = 0 AND [database_id] > 4;"
            "3.1" = "-- Automated via Get-DbaDbFile (tempdb ROWS count) + Get-DbaComputerSystem (CPU count). T-SQL: SELECT COUNT(*) FROM [tempdb].[sys].[database_files] WHERE [type] = 0;"
            "3.2" = "SELECT [name], [size], [growth], [is_percent_growth] FROM [tempdb].[sys].[database_files] WHERE [type] = 0 ORDER BY [name];"
            "3.3" = "SELECT DB_NAME([mf].[database_id]) AS [DatabaseName], [mf].[name], [mf].[growth], [mf].[is_percent_growth] FROM [sys].[master_files] [mf] WHERE [mf].[database_id] > 4 AND [mf].[is_percent_growth] = 1;"
            "3.4" = "-- Automated via Get-DbaDefaultPath. EXEC xp_instance_regread N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'DefaultData';"
            "4.1" = "SELECT [name], [is_disabled] FROM [sys].[server_principals] WHERE [sid] = 0x01;"
            "4.2" = "SELECT [name] FROM [sys].[server_principals] WHERE [sid] = 0x01;"
            "4.3" = "SELECT [name] FROM [sys].[databases] WHERE [is_trustworthy_on] = 1 AND [name] <> 'msdb' AND [database_id] > 4;"
            "4.4" = "-- Run per user database: SELECT [permission_name] FROM [sys].[database_permissions] WHERE [grantee_principal_id] = DATABASE_PRINCIPAL_ID('guest') AND [permission_name] = 'CONNECT';"
            "4.5" = "-- Automated via Get-DbaDbOrphanUser. T-SQL: SELECT [dp].[name] FROM [sys].[database_principals] [dp] LEFT JOIN [sys].[server_principals] [sp] ON [dp].[sid] = [sp].[sid] WHERE [dp].[type] IN ('S','U','G') AND [sp].[sid] IS NULL AND [dp].[principal_id] > 4;"
            "4.6" = "SELECT [permission_name], [state_desc], [class_desc] FROM [sys].[server_permissions] WHERE [grantee_principal_id] = SUSER_SID(N'public') AND [state_desc] LIKE 'GRANT%' AND NOT ([permission_name] = 'VIEW ANY DATABASE' AND [class_desc] = 'SERVER') AND NOT ([permission_name] = 'CONNECT' AND [class_desc] = 'ENDPOINT' AND [major_id] IN (2,3,4,5));"
            "1.10"= "-- Automated via Test-DbaBuild. Web: https://sqlserverupdates.com to find the latest CU for each major version."
            "1.11"= "-- Automated via Get-DbaService -ComputerName -Type Engine. Check StartName against SYSTEM/LocalSystem."
            "4.7" = "-- Automated via Get-DbaInstanceProperty -InstanceProperty LoginMode. SMO ServerLoginMode: 1 = Integrated (Windows-only), 2 = Mixed. T-SQL: SELECT CAST(SERVERPROPERTY('IsIntegratedSecurityOnly') AS INT) AS [WindowsAuthOnly];"
            "4.8" = "-- Automated via Test-DbaSpn -ComputerName. CMD: setspn -L <service-account> to view all registered SPNs."
            "4.9" = "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'xp_cmdshell';"
            "4.10"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'clr enabled';"
            "4.11"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'Ole Automation Procedures';"
            "4.12"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'Ad Hoc Distributed Queries';"
            "4.13"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'cross db ownership chaining';"
            "4.14"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'Database Mail XPs';"
            "4.15"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'remote access';"
            "4.16"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'scan for startup procs';"
            "4.17"= "SELECT [sp].[name], [sp].[type_desc] FROM [sys].[server_role_members] [srm] JOIN [sys].[server_principals] [sp] ON [srm].[member_principal_id] = [sp].[principal_id] JOIN [sys].[server_principals] [r] ON [srm].[role_principal_id] = [r].[principal_id] WHERE [r].[name] = 'sysadmin' AND [sp].[type] = 'S' AND [sp].[sid] <> 0x01;"
            "4.18"= "SELECT [name] FROM [sys].[databases] WHERE [owner_sid] = 0x01 AND [database_id] > 4;"
            "4.19"= "SELECT [ls].[name] AS [LinkedServer], [ll].[remote_name] FROM [sys].[linked_logins] [ll] JOIN [sys].[servers] [ls] ON [ll].[server_id] = [ls].[server_id] WHERE [ll].[remote_name] IS NOT NULL AND [ll].[uses_self_credential] = 0;"
            "4.20"= "SELECT [name], [is_policy_checked], [is_expiration_checked] FROM [sys].[sql_logins] WHERE [is_policy_checked] = 0 AND [sid] <> 0x01 AND [is_disabled] = 0;"
            "4.21"= "-- Registry via xp_instance_regread: HKLM\SOFTWARE\Microsoft\MSSQLServer\MSSQLServer\AuditLevel. 0=None, 1=Success, 2=Failure, 3=All"
            "4.22"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'priority boost';"
            "4.23"= "SELECT [name], [value_in_use] AS [RunningValue] FROM [sys].[configurations] WHERE [name] = 'lightweight pooling';"
            "4.24"= "-- Automated via Get-DbaNetworkCertificate -ComputerName. Certificate stored in Windows cert store (LocalMachine\My); thumbprint in SQL Server registry."
            "4.25"= "-- Automated via Get-DbaForceNetworkEncryption. Registry: HKLM\SOFTWARE\Microsoft\Microsoft SQL Server\<instance>\MSSQLServer\SuperSocketNetLib\ForceEncryption."
            "5.4" ="-- Automated via Get-DbaDiskSpace -ComputerName. SQL volumes identified from database file paths in sys.master_files."
            "5.1" = "-- Automated via Test-DbaDiskAllocation -ComputerName. CMD: fsutil fsinfo ntfsinfo <drive>: | findstr 'Bytes Per Cluster'."
            "5.2" = "SELECT [mf].[name], [mf].[physical_name], [mf].[type_desc] FROM [sys].[master_files] [mf] WHERE [mf].[database_id] > 4 ORDER BY [mf].[database_id], [mf].[type_desc];"
            "5.3" = "SELECT [mf].[name], [mf].[physical_name], [mf].[type_desc] FROM [sys].[master_files] [mf] WHERE [mf].[database_id] = DB_ID(N'tempdb') OR [mf].[database_id] > 4 ORDER BY [mf].[database_id], [mf].[type_desc];"
            "6.1" = "-- Automated via Measure-DbaDbVirtualLogFile. T-SQL: DBCC LOGINFO;"
            "6.2" = "-- Automated via Get-DbaLastGoodCheckDb. T-SQL: DBCC DBINFO() WITH TABLERESULTS; (find dbi_dbccLastKnownGood)"
            "6.3" = "SELECT [database_name], MAX([backup_finish_date]) AS [LastFullBackup] FROM [msdb].[dbo].[backupset] WHERE [type] = 'D' GROUP BY [database_name];"
            "6.4" = "-- Automated via Get-DbaLastBackup. T-SQL: SELECT [database_name], MAX([backup_finish_date]) FROM [msdb].[dbo].[backupset] WHERE [type] = 'L' GROUP BY [database_name];"
            "6.5" = "SELECT [name], [state_desc] FROM [sys].[databases] WHERE [state_desc] NOT IN ('ONLINE', 'OFFLINE') AND [is_in_standby] = 0;"
            "7.1" = "-- Automated via Get-DbaService -ComputerName -Type Agent. CMD: sc query SQLSERVERAGENT."
            "7.2" = "SELECT [name], [severity], [enabled] FROM [msdb].[dbo].[sysalerts] WHERE [severity] BETWEEN 17 AND 25 AND [enabled] = 1;"
            "7.3" = "SELECT [name], [message_id], [enabled] FROM [msdb].[dbo].[sysalerts] WHERE [message_id] = 825 AND [enabled] = 1;"
            "7.4" = "SELECT [name], [enabled] FROM [msdb].[dbo].[sysoperators] WHERE [enabled] = 1;"
            "7.5" = "SELECT [a].[name] FROM [msdb].[dbo].[sysalerts] [a] WHERE [a].[enabled] = 1 AND NOT EXISTS (SELECT 1 FROM [msdb].[dbo].[sysnotifications] [n] WHERE [n].[alert_id] = [a].[id]) ORDER BY [a].[name];"
            "7.6" = "-- Automated via Get-DbaXESession. T-SQL: SELECT [name], [state_desc], [startup_state_desc] FROM [sys].[dm_xe_sessions] WHERE [name] = 'system_health';"
            "1.12"= "-- Automated via Get-DbaDump (sys.dm_server_memory_dumps). Files are in the SQL Server LOG directory with .mdmp extension."
            "2.7" = "-- Automated via Test-DbaDbCollation. T-SQL: SELECT [name], [collation_name] FROM [sys].[databases] WHERE [collation_name] <> CAST(SERVERPROPERTY('Collation') AS nvarchar(128)) AND [database_id] > 4;"
            "6.6" = "SELECT DB_NAME([database_id]) AS [DatabaseName], [file_id], [page_id], [event_type], [error_count], [last_update_date] FROM [msdb].[dbo].[suspect_pages] WHERE [event_type] IN (1, 2, 3);"
            "6.7" = "-- Automated via Test-DbaIdentityUsage. T-SQL per table: SELECT IDENT_CURRENT('<table>'), IDENT_SEED('<table>'), IDENT_INCR('<table>')."
            "7.7" = "SELECT [j].[name], [jh].[run_date], [jh].[run_time], [jh].[message] FROM [msdb].[dbo].[sysjobhistory] [jh] JOIN [msdb].[dbo].[sysjobs] [j] ON [j].[job_id] = [jh].[job_id] WHERE [jh].[run_status] = 0 AND [jh].[step_id] = 0 ORDER BY [jh].[run_date] DESC, [jh].[run_time] DESC;"
            "7.8" = "SELECT [j].[name], [a].[start_execution_date], DATEDIFF(MINUTE, [a].[start_execution_date], GETDATE()) AS [RunMinutes] FROM [msdb].[dbo].[sysjobactivity] [a] JOIN [msdb].[dbo].[sysjobs] [j] ON [j].[job_id] = [a].[job_id] WHERE [a].[start_execution_date] IS NOT NULL AND [a].[stop_execution_date] IS NULL AND [a].[run_requested_date] IS NOT NULL ORDER BY [RunMinutes] DESC;"
            "7.9" = "-- Automated via Get-DbaDbStoredProcedure. Check for DatabaseBackup, DatabaseIntegrityCheck, IndexOptimize, CommandExecute in master. Install: Install-DbaMaintenanceSolution."
            "8.1" = "SELECT SERVERPROPERTY('IsHadrEnabled') AS [IsHadrEnabled];"
            "8.2" = "SELECT [name], [mirroring_state_desc], [mirroring_partner_name] FROM [sys].[databases] WHERE [mirroring_state_desc] IS NOT NULL AND [database_id] > 4;"
            "8.3" = "SELECT [ar].[replica_server_name], [rs].[connected_state_desc] FROM [sys].[dm_hadr_availability_replica_states] [rs] JOIN [sys].[availability_replicas] [ar] ON [ar].[replica_id] = [rs].[replica_id];"
            "8.4" = "SELECT [ar].[replica_server_name], [rs].[synchronization_health_desc] FROM [sys].[dm_hadr_availability_replica_states] [rs] JOIN [sys].[availability_replicas] [ar] ON [ar].[replica_id] = [rs].[replica_id];"
            "8.5" = "SELECT [ar].[replica_server_name], [ar].[failover_mode_desc], [ar].[availability_mode_desc] FROM [sys].[availability_replicas] [ar] JOIN [sys].[availability_groups] [ag] ON [ag].[group_id] = [ar].[group_id];"
            "8.6" = "SELECT [ars].[replica_server_name], [drs].[database_name], [drs].[secondary_lag_seconds] FROM [sys].[dm_hadr_database_replica_states] [drs] JOIN [sys].[availability_replicas] [ars] ON [ars].[replica_id] = [drs].[replica_id] WHERE [drs].[is_local] = 0 ORDER BY [drs].[secondary_lag_seconds] DESC;"
            "8.7" = "SELECT [ag].[name] AS [AGName], [l].[dns_name], [l].[port] FROM [sys].[availability_group_listeners] [l] JOIN [sys].[availability_groups] [ag] ON [ag].[group_id] = [l].[group_id];"
            "9.1" = "SELECT [primary_database] AS [Database], 'Primary' AS [Role] FROM [msdb].[dbo].[log_shipping_primary_databases] UNION ALL SELECT [secondary_database], 'Secondary' FROM [msdb].[dbo].[log_shipping_secondary_databases];"
            "9.2" = "-- Automated via Test-DbaDbLogShipStatus. T-SQL: SELECT [primary_database], [last_backup_date], [backup_threshold] FROM [msdb].[dbo].[log_shipping_monitor_primary];"
            "9.3" = "-- Automated via Test-DbaDbLogShipStatus. T-SQL: SELECT [secondary_database], [last_restored_date], [restore_threshold] FROM [msdb].[dbo].[log_shipping_monitor_secondary];"
            "9.4" = "-- Automated via Get-DbaDbLogShipError. T-SQL: SELECT [database_name], [agent_type], [action], [message] FROM [msdb].[dbo].[log_shipping_monitor_error_detail] ORDER BY [log_time] DESC;"
            "1.13"= "-- Automated via Get-DbaService -ComputerName -Type Browser. CMD: sc query SQLBrowser"
            "1.14"= "-- Automated via Test-DbaBuild (SupportedUntil property). Reference: https://learn.microsoft.com/lifecycle/products/?products=sql-server"
            "1.15"= "-- Manual review. Get-DbaService -ComputerName '<host>' -Type Engine,Agent | Select-Object DisplayName, StartName. MSA accounts end with '$'."
            "2.8" = "-- Automated via Get-DbaDbQueryStoreOption. T-SQL per DB: SELECT [actual_state_desc], [readonly_reason] FROM [sys].[database_query_store_options];"
            "2.9" = "SELECT [name], [containment_desc] FROM [sys].[databases] WHERE [database_id] > 4 AND [containment_desc] <> N'NONE'; SELECT [name], [value_in_use] FROM [sys].[configurations] WHERE [name] = 'contained database authentication';"
            "4.26"= "SELECT [d].[name], CASE WHEN [de].[database_id] IS NOT NULL THEN 'Enabled' ELSE 'Disabled' END AS [TDE_Status], [de].[encryption_state_desc] FROM [sys].[databases] [d] LEFT JOIN [sys].[dm_database_encryption_keys] [de] ON [de].[database_id] = [d].[database_id] WHERE [d].[database_id] > 4;"
            "4.27"= "-- Automated via Get-DbaInstanceAudit. T-SQL: SELECT [name], [type_desc], [on_failure_desc], [is_state_enabled] FROM [sys].[server_audits];"
            "4.28"= "-- Automated via Get-DbaTraceFlag. T-SQL: DBCC TRACESTATUS(-1) WITH NO_INFOMSGS; (returns active global trace flags)"
            "4.29"= "-- Manual review. Check Windows Firewall Advanced Security or network ACLs. Default SQL port: 1433. Named instance port: SELECT [local_tcp_port] FROM [sys].[dm_exec_connections] WHERE [session_id] = @@SPID;"
            "6.8" = "-- Manual review. Test-DbaLastBackup validates backup files. Get-DbaDbRestoreHistory shows recent restore history but does not confirm a full validation test."
            "7.10"= "-- Automated via Get-DbaDbMailProfile / Get-DbaDbMailAccount. T-SQL: SELECT [profile_id], [name] FROM [msdb].[dbo].[sysmail_profile]; SELECT [account_id], [name] FROM [msdb].[dbo].[sysmail_account];"
            "10.1"= "-- Manual review. No automated check possible. Confirm runbook location, last test date, and coverage of failover/restore procedures."
            "10.2"= "-- Manual review. Context: SELECT DISTINCT [program_name], [login_name], DB_NAME([database_id]) AS [Database] FROM [sys].[dm_exec_sessions] WHERE [is_user_process] = 1 AND [program_name] <> '' ORDER BY [program_name];"
        }
    }
    ...
    process {
        foreach ($instance in $SqlInstance) {
            ...
            if (ShouldRun "1") {
                ...
                try {
                    $retData19  = Get-ErrorLogRetention -ctx $connSplat
                    $logCount   = $retData19.Count
                    $splatCheck = @{
                        CheckId        = "1.9"
                        ...
                        CurrentValue   = $retData19.Display
                        ExpectedValue  = ">= 12"
            ...
            if (ShouldRun "4") {
                ...
                $saFailed = $false
                try {
                    $saData4  = Get-SaLogin -ctx $connSplat
                } catch {
                    $saFailed = $true
                    Write-Warning "[$instance] Get-SaLogin (SA SID pre-fetch) failed — DC-4.1/DC-4.2 skipped: $($_.Exception.Message)"
                }

                # DC-4.1 SA Login Disabled
                try {
                    if ($saFailed) { throw "SA login data not available — Get-SaLogin pre-fetch failed" }
                    $saLogin41  = $saData4.Login
                    $saName41   = if ($saLogin41) { $saLogin41.Name } else { "sa" }
                    $splatCheck = @{
                        CheckId        = "4.1"
                        ...
                        Status         = if ($null -eq $saLogin41) { "Manual" } elseif ($saLogin41.IsDisabled) { "Pass" } else { "Fail" }
                        CurrentValue   = if ($null -eq $saLogin41) { "No login with SA SID (0x01) found" } elseif ($saLogin41.IsDisabled) { "Disabled (name: $saName41)" } else { "Enabled (name: $saName41)" }
                ...
                try {
                    if ($saFailed) { throw "SA login data not available — Get-SaLogin pre-fetch failed" }
                    $saLogin42  = $saData4.Login
                    $saName42   = if ($saLogin42) { $saLogin42.Name } else { $null }
                    $splatCheck = @{
                        CheckId        = "4.2"
                        ...
                        Status         = if ($null -eq $saLogin42) { "Manual" } elseif ($saName42 -ne "sa") { "Pass" } else { "Fail" }
                        CurrentValue   = if ($null -eq $saLogin42) { "No login with SA SID (0x01) found" } else { $saName42 }
                ...
                # DC-4.4 Guest User CONNECT
                try {
                    $guestData4 = Get-GuestAccess -ctx $connSplat
                    $splatCheck = @{
                        CheckId        = "4.4"
                        ...
                        Status         = if ($guestData4.Count -eq 0) { "Pass" } else { "Fail" }
                        CurrentValue   = if ($guestData4.Count -gt 0) { $guestData4.DatabaseNames -join ", " } else { "None" }
                ...
                try {
                    $orphanData4 = Get-OrphanedUsers -ctx $connSplat
                    $orphanList  = $orphanData4.Orphans | ForEach-Object { "$($_.DatabaseName)\$($_.UserName)" }
                    $splatCheck  = @{
                        CheckId        = "4.5"
                        ...
                        Status         = if ($orphanData4.Count -eq 0) { "Pass" } else { "Fail" }
                ...
                try {
                    $pubPerms   = Invoke-DbaQuery @connSplat -Query $sql["4.6"]
                ...
                try {
                    $authData4  = Get-AuthMode -ctx $connSplat
                    # SMO ServerLoginMode: 1 = Integrated (Windows-only), 2 = Mixed
                    $winOnly    = [int]$authData4.LoginMode -eq 1
                    $splatCheck = @{
                        CheckId        = "4.7"
                ...
                try {
                    $spnTest    = @(Test-DbaSpn -ComputerName $computerName -EnableException -WarningAction SilentlyContinue)
...
"@
                ...
                try {
                    $sqlAuth4       = Get-SqlAuthLogins -ctx $connSplat
                    $noPolicyLogins = @($sqlAuth4.Logins | Where-Object { -not $_.PasswordPolicyEnforced })
...
"@
                ...
                try {
                    ...
                    $splatCheck = @{
                        ...
                        Category       = "Security"
                        AssessmentType = "Manual"
                        Priority       = $priority["4.26"]
                        Status         = "Manual"
                        CurrentValue   = if ($tdeData4.EncryptedCount -gt 0) {
                            "TDE enabled: $($tdeData4.EncryptedNames -join ', ')"
